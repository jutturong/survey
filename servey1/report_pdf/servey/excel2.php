...
header('Content-Disposition: attachment; filename="department.xls"');#ชื่อไฟล์
...
//$dep=123;  //test  department
//  `tb_employee`.`id_department`=$dep
...
<TD width="50" align="center">ร้อยละ</TD>
    ...
    //left  join  `tb_record1`   on  `tb_record1`.`id_employee_main` =`tb_employee`.`id_employee`    and   `tb_record1`.`dmy_insert`   between  $b_year   and  $e_year  "; 
    ////  `tb_employee`.`id_department`=$dep
    $strsex="  SELECT   *   FROM    `tb_employee`   left   join  `tb_record1`   on  `tb_record1`.`id_employee_main` =`tb_employee`.`id_employee`   where  `tb_employee`.`id_department`=$dep   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'   ";   // `tb_employee`.`id_sex`  >  0
...
<TR>
   ...
   //`tb_employee`
    ...
    //and  `tb_employee`.`id_department`=$dep 
     $strsex_man=" SELECT * FROM `tb_employee`  left  join  `tb_record1`   on  `tb_record1`.`id_employee_main` =`tb_employee`.`id_employee`    where  `tb_employee`.`id_sex` = 1  and  `tb_employee`.`id_department`=$dep     and   `tb_record1`.`dmy_insert`   between    '$b_year'   and  '$e_year'   ";
...
//$b_year= $cur_year_ph."-01-01";
   ...
   //`tb_employee`
       //and  `tb_employee`.`id_department`=$dep 
     $strsex_w=" SELECT * FROM `tb_employee`   left  join  `tb_record1`   on  `tb_record1`.`id_employee_main` =`tb_employee`.`id_employee`   where  `tb_employee`.`id_sex` = 2   and  `tb_employee`.`id_department`=$dep    and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'   ";
   ...
   //`tb_employee`
    //and  `tb_employee`.`id_department`=$dep 
       $str_age="  SELECT *  FROM `tb_record1` left  join  `tb_employee`   on  `tb_record1`.`id_employee_main` =`tb_employee`.`id_employee`  where    `tb_employee`.`id_department`=$dep  and   `tb_record1`.`dmy_insert`   between    '$b_year'   and  '$e_year'   ";
   ...
   //`tb_employee`
       //and  `tb_employee`.`id_department`=$dep 
     // บรรทัดนี้ ยังไม่เพิ่มเติมให้ระวัง
     //and  `tb_employee`.`id_department`=$dep 
       $str_age="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where        `tb_employee`.`id_sex` =1    and  `tb_employee`.`id_department`=$dep    and  `tb_record1`.`dmy_insert`   between    '$b_year'   and  '$e_year'    ";
       $query_age=mysql_query($str_age);
        $c_age=0;
       while($row=  mysql_fetch_object($query_age))
       {
   ...
   //`tb_employee`
    // $c_age += $row->age;  //อายุรวมทั้งหมด
     //and  `tb_employee`.`id_department`=$dep 
      $str_age21="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =1    and  `tb_employee`.`id_department`=$dep    and  `tb_record1`.`age` <= 21   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'  ";
   ...
   //`tb_employee`
      //and  `tb_employee`.`id_department`=$dep 
      $str_age2="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =1   and  `tb_employee`.`id_department`=$dep   and  `tb_record1`.`age` between  21 and 30   and   `tb_record1`.`dmy_insert`   between    '$b_year'   and  '$e_year'   ";
    ...
    <?php // 31-40
    //and  `tb_employee`.`id_department`=$dep
      $str_age3="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =1  and  `tb_employee`.`id_department`=$dep    and  `tb_record1`.`age` between  31 and 40   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'  ";
    ...
    <?php // 41-50
    //and  `tb_employee`.`id_department`=$dep
      $str_age4="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =1  and  `tb_employee`.`id_department`=$dep   and  `tb_record1`.`age` between  41 and 50   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'   ";
    ...
    <!----------    เพศชาย 41-50 ------------------->
     ...
     <?php // 51-60
     //and  `tb_employee`.`id_department`=$dep
      $str_age5="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =1    and  `tb_employee`.`id_department`=$dep   and  `tb_record1`.`age` between  51 and 60   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'   ";
    ...
    <?php //อายุทั้งหมดของเพศหญิง  เกิด  error ขึ้น
     //$b_year= $cur_year_ph."-01-01";
    //$e_year= $cur_year_ph."-12-31";
    //left  join  `tb_record1`   on  `tb_record1`.`id_employee_main` =`tb_employee`.`id_employee`    and   `tb_record1`.`dmy_insert`   between  $b_year   and  $e_year  "; 
   //`tb_employee`
    //and  `tb_employee`.`id_department`=$dep
       $str_age_w="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where       `tb_employee`.`id_sex` =2     and    `tb_employee`.`id_department`=$dep    and  `tb_record1`.`dmy_insert`   between    '$b_year'   and  '$e_year'    ";
  ...
  <!-- หญิง น้อยกว่า 21 ปี -->
   ...
   //`tb_employee`
    // $c_age += $row->age;  //อายุรวมทั้งหมด
   //and  `tb_employee`.`id_department`=$dep
      $str_age21w="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =2    and   `tb_employee`.`id_department`=$dep    and  `tb_record1`.`age` <= 21   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'   ";
 ...
 <?php  //เพศชายอายุน้อยกว่า 21-30 ปี
   ...
   //`tb_employee`
 //and  `tb_employee`.`id_department`=$dep
      $str_age2w="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =2  and   `tb_employee`.`id_department`=$dep    and  `tb_record1`.`age` between  21 and 30   and   `tb_record1`.`dmy_insert`   between    '$b_year'   and  '$e_year'   ";
 ...
 <?php // 31-40
 //and  `tb_employee`.`id_department`=$dep
      $str_age3w="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =2   and   `tb_employee`.`id_department`=$dep    and  `tb_record1`.`age` between  31 and 40   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'   ";
   ...
   <!----------    เพศหญิง 41-50 ------------------->
    ...
    <!----------    เพศชาย 41-50 ------------------->
     ...
     <?php // 51-60
     //and  `tb_employee`.`id_department`=$dep
      $str_age5w="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =2    and   `tb_employee`.`id_department`=$dep   and  `tb_record1`.`age` between  51 and 60   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'   ";
    ...
    <?php // มากกว่า 60 ปี
   // and  `tb_employee`.`id_department`=$dep
      $str_age6w="  SELECT *  FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main` where   `tb_employee`.`id_sex` =2   and   `tb_employee`.`id_department`=$dep   and  `tb_record1`.`age` >  60   and   `tb_record1`.`dmy_insert`   between   '$b_year'   and  '$e_year'  ";
    ...
    <!--  ดัชนีมวลกาย -->
     <?php
     //and  `tb_employee`.`id_department`=$dep
      $str_bmi="  SELECT   *   FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where       `tb_employee`.`id_department`=$dep   and   `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
 ...
 //and  `tb_employee`.`id_department`=$dep
    ...
    $str_bmi1="  SELECT   *   FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where    `tb_employee`.`id_department`=$dep   and   `tb_record1`.`BMI`  < 18.5    and     `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>       
 ...
 <?php //error
 // `tb_employee`.`id_department`=$dep  and 
    $str_bmi2="  SELECT   *   FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where     `tb_employee`.`id_department`=$dep  and      `tb_record1`.`BMI`  >= 18.5    and   `tb_record1`.`BMI` <= 22.9  and     `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>       
   ...
   <!--  ดัชนีมวลกาย  ผอม น้อยกว่า 18.5-22.9 -->  
    ...
    <!--  ดัชนีมวลกาย  ผอม น้อยกว่า 23.0-24.9 -->  
     <?php  // error code
     // `tb_employee`.`id_department`=$dep  and 
    $str_bmi3="  SELECT   *   FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where  `tb_employee`.`id_department`=$dep   and   `tb_record1`.`BMI`  >= 23.0    and   `tb_record1`.`BMI` <= 24.9  and     `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>   
    ...
    <!--  ดัชนีมวลกาย  อ้วน ( 25.0 - 29.9 ) -->  
     <?php //  error
    //  `tb_employee`.`id_department`=$dep  and 
    $str_bmi4="  SELECT   *   FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where     `tb_employee`.`id_department`=$dep   and   `tb_record1`.`BMI`  >= 25.0    and   `tb_record1`.`BMI` <= 29.9  and     `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>  
   ...
   <!-- อ้วนอันตราย ( มากกว่าหรือเท่ากับ 30 )  --> 
    <?php  // error
    // `tb_employee`.`id_department`=$dep  and 
    $str_bmi5="  SELECT   *   FROM `tb_record1`  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where     `tb_employee`.`id_department`=$dep   and   `tb_record1`.`BMI`  >= 30     and     `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>
   ...
   <!-- อ้วนอันตราย ( มากกว่าหรือเท่ากับ 30 )  --> 
    ...
    $str_AR="  SELECT   *   FROM `tb_record1`   left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where     `tb_employee`.`id_department`=$dep   and     `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>
   ...
   // `tb_employee`.`id_department`=$dep  and 
      //  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`     where        `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year' 
    $str_m="  SELECT   *   FROM `tb_record1`   left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where      `tb_employee`.`id_sex` = 1   and   `tb_employee`.`id_department`=$dep    and   `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>
 ...
 <?php
      //  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`     where        `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year' 
    $str_m="  SELECT   *   FROM `tb_record1`   left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where      `tb_employee`.`id_sex` = 1  and   `tb_employee`.`id_department`=$dep   and   `tb_record1`.`AR` <= 90  and   `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
      $query_m = mysql_query( $str_m );
      $c_ar_m=0; 
      while($row=  mysql_fetch_object($query_m))
       {
...
?>  
   ...
   <?php
      //  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`     where        `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year' 
    $str_m2="  SELECT   *   FROM `tb_record1`   left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where      `tb_employee`.`id_sex` = 1   and   `tb_employee`.`id_department`=$dep    and   `tb_record1`.`AR` > 90  and   `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>  
    <tr>
        ...
        <?php
      //  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`     where        `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year' 
    $str_w="  SELECT   *   FROM `tb_record1`   left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where      `tb_employee`.`id_sex` = 2   and   `tb_employee`.`id_department`=$dep   and   `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>
   ...
   <?php
      //  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`     where        `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year' 
    $str_w1="  SELECT   *   FROM `tb_record1`   left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where      `tb_employee`.`id_sex` = 2  and   `tb_employee`.`id_department`=$dep   and   `tb_record1`.`AR` <= 80  and   `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
?>  
     ...
     <?php
      //  left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`     where        `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year' 
    $str_w2="  SELECT   *   FROM `tb_record1`   left  join  `tb_employee`   on   `tb_employee`.`id_employee`  =  `tb_record1`.`id_employee_main`   where      `tb_employee`.`id_sex` = 2  and   `tb_employee`.`id_department`=$dep   and   `tb_record1`.`AR` > 80  and   `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'  ";
...
<TD width="50" align="center">ร้อยละ</TD>
  ...
  <?php //Error
      ...
      //diag_detail
  //and   `tb_employee`.`id_department`=$dep 
       $str_disease =" SELECT * FROM   `tb_record1`  "
               ...
               . "where    `tb_employee`.`id_department`=$dep    and     `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'   ";
          ...
          while($row=  mysql_fetch_object( $query_disease))
          {
             ...
             $id_disease=$row->id_disease;
              ...
              //tb_record1`.`diag_detail` = $id_disease   and
                $str_dis="  SELECT  *   FROM `tb_record1`   "
                        ."  left  join   `tb_employee`  on    `tb_record1`.`id_employee_main`    = `tb_employee`.`id_employee`    "   
                        . " where   `tb_employee`.`id_department`=$dep   and    diag_detail =  $id_disease   and   `tb_record1`.`dmy_insert`   between  '$b_year'   and  '$e_year'    ";
